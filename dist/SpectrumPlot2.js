(function (global, factory) {
    ...
    (factory((global.SpectrumPlot2 = global.SpectrumPlot2 || {})));
...
var reverseNames = {};
for (var name in index$4) {
   reverseNames[index$4[name]] = name;
}
...
Color.prototype.setValues = function (space, vals) {
	...
	for (var sname in spaces) {
		if (sname !== space) {
			values[sname] = index$2[space][sname](values[space]);
		}
	}
...
var core_helpers = function(Chart) {
		...
		helpers.each(extension, function(value, key) {
			if (key === 'xAxes' || key === 'yAxes') {
				...
				if (base.hasOwnProperty(key)) {
					helpers.each(value, function(valueObj, index$$1) {
						var axisType = helpers.getValueOrDefault(valueObj.type, key === 'xAxes' ? 'category' : 'linear');
						var axisDefaults = Chart.scaleService.getScaleDefaults(axisType);
						if (index$$1 >= base[key].length || !base[key][index$$1].type) {
							base[key].push(helpers.configMerge(axisDefaults, valueObj));
						} else if (valueObj.type && valueObj.type !== base[key][index$$1].type) {
							// Type changed. Bring in the new defaults before we bring in valueObj so that valueObj can override the correct scale defaults
							base[key][index$$1] = helpers.configMerge(base[key][index$$1], axisDefaults, valueObj);
						} else {
							// Type is the same
							base[key][index$$1] = helpers.configMerge(base[key][index$$1], valueObj);
						}
					});
				} else {
					base[key] = [];
	...
	helpers.getValueAtIndexOrDefault = function(value, index$$1, defaultValue) {
		if (value === undefined || value === null) {
			return defaultValue;
		}

		if (helpers.isArray(value)) {
			return index$$1 < value.length ? value[index$$1] : defaultValue;
		}
	...
	helpers.nextItem = function(collection, index$$1, loop) {
		if (loop) {
			return index$$1 >= collection.length - 1 ? collection[0] : collection[index$$1 + 1];
		}
		return index$$1 >= collection.length - 1 ? collection[collection.length - 1] : collection[index$$1 + 1];
	};
	helpers.previousItem = function(collection, index$$1, loop) {
		if (loop) {
			return index$$1 <= 0 ? collection[collection.length - 1] : collection[index$$1 - 1];
		}
		return index$$1 <= 0 ? collection[0] : collection[index$$1 - 1];
	};
...
var platform_dom = function(Chart) {
	var helpers = Chart.helpers;
...
var platform = function(Chart) {
	...
	Chart.helpers.extend(Chart.platform, platform_dom(Chart));
};

var core_canvasHelpers = function(Chart) {
...
var core_element = function(Chart) {
...
var core_interaction = function(Chart) {
	var helpers = Chart.helpers;
...
var element_arc = function(Chart) {
...
var element_line = function(Chart) {
...
var element_point = function(Chart) {
...
var element_rectangle = function(Chart) {
...
var plugin_filler = function(Chart) {
	/**
...
core_ticks(Chart);
core_scale(Chart);
core_interaction(Chart);
core_tooltip(Chart);

element_arc(Chart);
element_line(Chart);
element_point(Chart);
element_rectangle(Chart);
...
var line = function(Chart) {
	var chartHelpers = Chart.helpers;
	var helpers$$1 = helpers(Chart);
	...
	var LineAnnotation = Chart.Annotation.Element.extend({
		...
		configure: function() {
			...
			var pixel, endPixel;
			if (scale) {
				pixel = helpers$$1.isValid(options.value) ? scale.getPixelForValue(options.value) : NaN;
				endPixel = helpers$$1.isValid(options.endValue) ? scale.getPixelForValue(options.endValue) : pixel;
			}
...
var box = function(Chart) {
	...
	var BoxAnnotation = Chart.Annotation.Element.extend({
		setDataLimits: function() {
			...
			if (xScale) {
				min = helpers$$1.isValid(options.xMin) ? options.xMin : xScale.getPixelForValue(chartArea.left);
				max = helpers$$1.isValid(options.xMax) ? options.xMax : xScale.getPixelForValue(chartArea.right);
			...
			if (yScale) {
				min = helpers$$1.isValid(options.yMin) ? options.yMin : yScale.getPixelForValue(chartArea.bottom);
				max = helpers$$1.isValid(options.yMax) ? options.yMax : yScale.getPixelForValue(chartArea.top);
		...
		configure: function() {
			...
			if (xScale) {
				min = helpers$$1.isValid(options.xMin) ? xScale.getPixelForValue(options.xMin) : chartArea.left;
				max = helpers$$1.isValid(options.xMax) ? xScale.getPixelForValue(options.xMax) : chartArea.right;
				left = Math.min(min, max);
				right = Math.max(min, max);
			}

			if (yScale) {
				min = helpers$$1.isValid(options.yMin) ? yScale.getPixelForValue(options.yMin) : chartArea.bottom;
				max = helpers$$1.isValid(options.yMax) ? yScale.getPixelForValue(options.yMax) : chartArea.top;
		...
		if (e.type === 'mousemove') {
			if (element && !element.hovering) {
				...
				['mouseenter', 'mouseover'].forEach(function(eventName) {
					var eventHandlerName = helpers$$1.getEventHandlerName(eventName);
					var hoverEvent = helpers$$1.createMouseEvent(eventName, e); // recreate the event to match the handler
...
var annotation = function(Chart) {
	var chartHelpers = Chart.helpers;

	var helpers$$1 = helpers(Chart);
	var events$$1 = events(Chart);

	var annotationTypes = Chart.Annotation.types;

	function setAfterDataLimitsHook(axisOptions) {
		helpers$$1.decorate(axisOptions, 'afterDataLimits', function(previous, scale) {
			if (previous) previous(scale);
			helpers$$1.adjustScaleRange(scale);
		});
	}
	...
	function draw(drawTime) {
		return function(chartInstance, easingDecimal) {
			...
			helpers$$1.elements(chartInstance)
				.filter(function(element) {
	...
	return {
		beforeInit: function(chartInstance) {
			...
			var ns = chartInstance.annotation = {
				elements: {},
				options: helpers$$1.initConfig(chartOptions.annotation || {}),
				onDestroy: [],
				firstRun: true,
				supported: false
			};
		...
		beforeUpdate: function(chartInstance) {
			...
			if (!ns.firstRun) {
				ns.options = helpers$$1.initConfig(chartInstance.options.annotation || {});
			} else {
				ns.firstRun = false;
			}
			...
			ns.options.annotations.forEach(function(annotation) {
				var id = annotation.id || helpers$$1.objectId();
		...
		afterScaleUpdate: function(chartInstance) {
			helpers$$1.elements(chartInstance).forEach(function(element) {
				element.configure();
			});
		},
		...
		afterInit: function(chartInstance) {
			...
			if (chartHelpers.isArray(watchFor) && watchFor.length > 0) {
				...
				var eventHandler = events$$1.dispatcher.bind(chartInstance);
				events$$1.collapseHoverEvents(watchFor).forEach(function(eventName) {
...
var Chart$2 = chart;
Chart$2 = typeof(Chart$2) === 'function' ? Chart$2 : window.Chart;

// Configure plugin namespace
Chart$2.Annotation = Chart$2.Annotation || {};

Chart$2.Annotation.drawTimeOptions = {
	afterDraw: 'afterDraw',
	afterDatasetsDraw: 'afterDatasetsDraw',
	beforeDatasetsDraw: 'beforeDatasetsDraw'
};

Chart$2.Annotation.defaults = {
...
Chart$2.Annotation.labelDefaults = {
	backgroundColor: 'rgba(0,0,0,0.8)',
	fontFamily: Chart$2.defaults.global.defaultFontFamily,
	fontSize: Chart$2.defaults.global.defaultFontSize,
	fontStyle: 'bold',
	fontColor: '#fff',
	xPadding: 6,
	yPadding: 6,
...
Chart$2.Annotation.Element = element(Chart$2);

Chart$2.Annotation.types = {
	line: line(Chart$2),
	box: box(Chart$2)
};

var annotationPlugin = annotation(Chart$2);

Chart$2.pluginService.register(annotationPlugin);
...
exports.SpectrumPlot2 = SpectrumPlot2;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=SpectrumPlot2.js.map
